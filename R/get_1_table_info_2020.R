...
#'
#' This function is to be used for the pdf reports from 2020 that only have one
#' table in the pdf (Jan 2020 reports have one table but Feb 2020 reports have two). If the
#' report being scrapped has two tables see \code{get_two_tables_2020}. The
#' arguments for this function are the same as the outputs for \code{get_table_info_old
#' } and can be obtained from the following functions: \code{get_facility_id}, \code{get_CNN
#' }, \code{get_facility_name}, \code{get_report_period}, \code{get_comparison_group},
#' \code{get_run_date}, \code{get_city_and_state},\code{get_version_number}.
...
#'    get_1_table_info_2020(raw_text, Facility_ID = "MS0004", CCN = "123456", Facility_Name =
...
#'
#'    get_1_table_info_2020(raw_text, Facility_ID = facility_id, CCN = ccn, Facility_Name =
